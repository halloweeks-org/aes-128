...
static void AES_encrypt_init(const uint8_t *key, uint32_t *rk) {
	rk[0] = GETU32(key + 0);
	rk[1] = GETU32(key + 4);
	rk[2] = GETU32(key + 8);
	rk[3] = GETU32(key + 12);
	...
	for (uint8_t index = 4; index < 44; index += 4) {
		rk[index] = rk[index - 4] ^ 
			(Te4[(rk[index - 1] >> 16) & 0xff] & 0xff000000) ^ 
			(Te4[(rk[index - 1] >>  8) & 0xff] & 0x00ff0000) ^
			(Te4[(rk[index - 1] >>  0) & 0xff] & 0x0000ff00) ^
			(Te4[(rk[index - 1] >> 24) & 0xff] & 0x000000ff) ^ 
				rcon[index / 4 - 1];
		rk[index + 1] = rk[index - 3] ^ rk[index];
		rk[index + 2] = rk[index - 2] ^ rk[index + 1];
		rk[index + 3] = rk[index - 1] ^ rk[index + 2];
	}
}

static void AES_decrypt_init(const uint8_t* key, uint32_t *rk) {
    uint32_t temp;
    ...
    for (uint8_t index = 4; index < 40; index += 4) {
        rk[index] =
            Td0[Te4[(rk[index] >> 24) & 0xff] & 0xff] ^
            Td1[Te4[(rk[index] >> 16) & 0xff] & 0xff] ^
            Td2[Te4[(rk[index] >>  8) & 0xff] & 0xff] ^
            Td3[Te4[(rk[index] >>  0) & 0xff] & 0xff];
        rk[index + 1] =
            Td0[Te4[(rk[index + 1] >> 24) & 0xff] & 0xff] ^
            Td1[Te4[(rk[index + 1] >> 16) & 0xff] & 0xff] ^
            Td2[Te4[(rk[index + 1] >>  8) & 0xff] & 0xff] ^
            Td3[Te4[(rk[index + 1] >>  0) & 0xff] & 0xff];
        rk[index + 2] =
            Td0[Te4[(rk[index + 2] >> 24) & 0xff] & 0xff] ^
            Td1[Te4[(rk[index + 2] >> 16) & 0xff] & 0xff] ^
            Td2[Te4[(rk[index + 2] >>  8) & 0xff] & 0xff] ^
            Td3[Te4[(rk[index + 2] >>  0) & 0xff] & 0xff];
        rk[index + 3] =
            Td0[Te4[(rk[index + 3] >> 24) & 0xff] & 0xff] ^
            Td1[Te4[(rk[index + 3] >> 16) & 0xff] & 0xff] ^
            Td2[Te4[(rk[index + 3] >>  8) & 0xff] & 0xff] ^
            Td3[Te4[(rk[index + 3] >>  0) & 0xff] & 0xff];
    }
}


static void AES_encrypt(const uint8_t plaintext[AES_BLOCK_SIZE], uint8_t ciphertext[AES_BLOCK_SIZE], const uint32_t *rk)
{
	uint32_t s0, s1, s2, s3, t0, t1, t2, t3;
	
	// Map byte array block to cipher state and add the initial round key:
	...
	s0 = 
		...
		(Te4[(t3 >>  0) & 0xff] & 0x000000ff) ^ rk[40];
	...
	s1 =	 
		...
		(Te4[(t0 >>  0) & 0xff] & 0x000000ff) ^ rk[41];
	...
	s2 = 
		...
		(Te4[( t1 >>  0) & 0xff] & 0x000000ff) ^ rk[42];
	...
	s3 = 
		...
		(Te4[( t2 >>  0 ) & 0xff] & 0x000000ff) ^ rk[43];
	PUTU32( ciphertext + 12, s3 );
}

static void AES_decrypt(const uint8_t ciphertext[AES_BLOCK_SIZE], uint8_t plaintext[AES_BLOCK_SIZE], const uint32_t *rk) {
	uint32_t s0, s1, s2, s3, t0, t1, t2, t3;
...
static void AES_cbc_encrypt(const uint8_t *data, uint32_t size, uint8_t *out, const uint32_t *rk, const uint8_t *iv) {
    ...
    for (uint32_t i = 0; i < size; i += 16) {
        uint8_t block[16];
        for (int j = 0; j < 16; j++) {
            block[j] = data[i + j] ^ prev_block[j];  // XOR with the previous ciphertext block
        }

        // Encrypt the XORed block using AES-128 ECB
        AES_encrypt(block, out + i, rk);

        // Update the previous block with the ciphertext
...
static void AES_cbc_decrypt(const uint8_t *data, uint32_t size, uint8_t *out, const uint32_t *rk, const uint8_t *iv) {
    ...
    for (uint32_t i = 0; i < size; i += 16) {
        uint8_t block[16];
        AES_decrypt(data + i, block, rk);  // Decrypt the current ciphertext block

        for (int j = 0; j < 16; j++) {
            out[i + j] = block[j] ^ prev_block[j];  // XOR with the previous ciphertext block
        }
